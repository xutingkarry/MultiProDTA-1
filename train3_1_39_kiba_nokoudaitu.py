...
from models.graphTran_cnnpro3d_39_nokoudaitu import GraphTransformer

from utils6_78_3_kiba import *
# import ci_cython
file_path = "result/train_kiba_gin_nokoudaitu.txt"
...
def train(model, device, train_loader, optimizer, epoch):#(GCN,cuda:0,训练的数据，优化器，轮次增加)
    ...
    for batch_idx,(data,datapro,datapro_3d) in enumerate(train_loader):
        ...
        if batch_idx % LOG_INTERVAL == 0:
            ...
            str = 'Train epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(epoch,
                                                                           batch_idx * len(data.x),
                                                                           len(train_loader.dataset),
                                                                           100. * batch_idx / len(train_loader),
                                                                           loss.item()) + "\n"
...
def main():
    ...
    for dataset in datasets:#数据准备
        ...
        processed_data_file_train_drug = 'datasets/datasets/datasets/processed/' + dataset + '_train_data_mol_78_1_kiba.pt'
        processed_data_file_test_drug = 'datasets/datasets/datasets/processed/' + dataset + '_test_data_mol_78_1_kiba.pt'
        processed_data_file_train_pro = 'datasets/datasets/datasets/processed/' + dataset + '_train_data_pro_graph_3_kiba.pt'
        processed_data_file_test_pro = 'datasets/datasets/datasets/processed/' + dataset + '_test_data_pro_graph_3_kiba.pt'
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                if ret[1] < best_mse:
                    ...
                    print('rmse improved at epoch ', best_epoch, '; best_mse,best_ci:', best_mse, best_ci,
                          ...
                          model_st, dataset)
                    print('')
                    output_str1 = 'rmse improved at epoch {}; best_mse,best_ci: {}, {} best_rm2: {} {} {}'.format(
                        best_epoch, best_mse, best_ci, best_rm2, model_st, dataset)+"\n"
                    with open(file_path, "a") as file:
                    ...
                    print('MSE:', ret[1], 'CI:', ret[-1], 'RM2:', ret[-3], 'No improvement since epoch ', best_epoch,
                          ...
                          model_st, dataset)
                    print('')
                    output_str = 'MSE: {} CI: {} RM2: {} No improvement since epoch {}; best_mse,best_ci: {}, {} best_rm2: {} {} {}'.format(
                        ret[1], ret[-1], ret[-3], best_epoch, best_mse, best_ci, best_rm2, model_st, dataset)+"\n"
