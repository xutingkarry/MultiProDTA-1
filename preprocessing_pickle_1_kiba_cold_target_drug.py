...
from utils6_78_2_kiba_cold_target_drug import *
...
for dt_name in ['kiba']:
    opts = ['train', 'test']
    for opt in opts:
        df = pd.read_csv(process_dir +dt_name+'/' +'KIBA' + '_cold_drug_' + opt + '_42.csv')
...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        # 训练集
        df = pd.read_csv(process_dir + dataset+'/'+'KIBA' + '_cold_drug_train_42.csv')
        ...
        # 验证集
        df = pd.read_csv(process_dir + dataset+'/'+'KIBA' + '_cold_drug_test_42.csv')
