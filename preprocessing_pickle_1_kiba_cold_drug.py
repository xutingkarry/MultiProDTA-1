...
from utils6_78_2_kiba_cold_drug import *
...
def sequence2graph(target_key, target_sequence):
    target_edge_index = []
    ...
    #在这里插入蛋白质3d坐标
    pdb_file='datasets/datasets/datasets/kiba/pdb/AF-'+target_key+'-F1-model_v4.pdb'
    X_ca_3d,node_s_3d,node_v_3d,edge_s_3d,edge_v_3d,edge_index_3d = pdb_to_graph(pdb_file, max_seq_len=10, device='cuda:0')
...
for dt_name in ['kiba']:
    opts = ['train', 'test']
    for opt in opts:
        df = pd.read_csv(process_dir +dt_name+'/' +'KIBA' + '_cold_target_drug_' + opt + '_42.csv')
...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        # 训练集
        df = pd.read_csv(process_dir + dataset+'/'+'KIBA' + '_cold_target_drug_train_42.csv')
        ...
        # 验证集
        df = pd.read_csv(process_dir + dataset+'/'+'KIBA' + '_cold_target_drug_test_42.csv')
