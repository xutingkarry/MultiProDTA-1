...
def adj(edge_index,ptr):
    bsz = len(ptr) - 1
    num_nodes = torch.diff(ptr).max().item()
    adjacency_matrix = torch.zeros(num_nodes, num_nodes)
    for i in range(edge_index.shape[1]):
        start_node = int(edge_index[0][i])
        end_node = int(edge_index[1][i])
        adjacency_matrix[start_node][end_node] = 1
        adjacency_matrix[end_node][start_node] = 1  # 如果是无向图，需要设置对称位置为1
