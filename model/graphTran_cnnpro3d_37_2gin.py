...
class GraphTransformer(nn.Module):
    def __init__(self, in_size=78, in_size_prot=33,num_class=2, d_model=128, num_heads=8,num_features_xc=92,
        ...
        self.gin_conv1 = GINConv(nn.Sequential(nn.Linear(33, 128), nn.ReLU(),
                                               nn.Linear(128, 128)))
        self.gin_conv2 = GINConv(nn.Sequential(nn.Linear(128, 128), nn.ReLU(),
                                                nn.Linear(128, 128)))
...
#对药物分子图进行处理
        ...
        pro_graph = self.gin_conv1(x_pro, data_pro.edge_index)
        pro_graph=self.relu(self.pro_graph_bn(pro_graph))

        pro_graph = self.gin_conv2(pro_graph, data_pro.edge_index)
        pro_graph = self.relu(self.pro_graph_bn(pro_graph))
        ...
        pro_3d_graph=self.pro_3d_conv1(data_pro_3d.x_3d,data_pro_3d.edge_index_3d)
        pro_3d_graph=self.relu(self.pro_3d_bn(pro_3d_graph))

        pro_3d_graph = self.pro_3d_conv2(pro_3d_graph, data_pro_3d.edge_index_3d)
        pro_3d_graph = self.relu(self.pro_3d_bn(pro_3d_graph))
