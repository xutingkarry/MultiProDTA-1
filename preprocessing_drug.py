...
def read_data(filename):
    ...
    drugs, prots, Y = list(df['smiles']),list(df['protein_seqence']),list(df['protein_ID']),list(df['affinity'])
...
for dt_name in ['davis']:
    ...
    for opt in opts:
        ...
        prot_seq+=list(df['protein_seqence'])
        pro_id+=list(df['protein_ID'])
...
for seq,key in zip(prot_seq,pro_id):
    p=sequence2graph(key,seq,process_dir)
    prot_graph[seq]=p
...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        # 训练集
        df = pd.read_csv(process_dir+ dataset + 'train5.csv')
        train_drugs, train_prots, train_Y = list(df['smiles']), list(df['protein_sequence']), list(
            df['affinity'])
        train_drugs, train_prots, train_Y = np.asarray(train_drugs), np.asarray(train_prots), np.asarray(train_Y)
        #验证集
        df = pd.read_csv(process_dir+ dataset + '_test5.csv')
        valid_drugs, valid_prots, valid_Y = list(df['smiles']), list(df['protein_sequence']), list(
            df['affinity'])
        test_drugs, test_prots, test_Y = np.asarray(valid_drugs), np.asarray(valid_prots), np.asarray(valid_Y)

        train_data = DTADataset(root='datasets/datasets/datasets', dataset=dataset + '_' + 'train', drug_smiles=train_drugs, target_sequence=train_prots, y=train_Y,
                                    smile_graph=smile_graph,target_graph=None)
        test_data = DTADataset(root='datasets/datasets/datasets', dataset=dataset + '_' + 'test', drug_smiles=test_drugs, target_sequence=test_prots, y=test_Y,
                                smile_graph=smile_graph, target_graph=None)



        train_data_pro = DTADataset1(root='datasets/datasets/datasets', dataset=dataset + '_' + 'train', drug_smiles=train_drugs,
                                    target_sequence=train_prots, y=train_Y,
                                    smile_graph=None, target_graph=prot_graph)

        test_data_pro = DTADataset1(root='datasets/datasets/datasets', dataset=dataset + '_' + 'test', drug_smiles=test_drugs,
...
#     train_csv = process_dir+dataset + '/raw/davis_train5.csv'
#     test_csv = process_dir+dataset + '/raw/davis_test5.csv'
#
#     train_drugs, train_prots, train_prot_id,train_Y = read_data(train_csv)
#     test_drugs, test_prots,train_prot_id,test_Y = read_data(test_csv)
#
#     train_drugs, train_prots, train_prot_id,train_Y = np.asarray(train_drugs), np.asarray(train_prots), np.asarray(train_Y)
#     test_drugs, test_prots, train_prot_id,test_Y = np.asarray(test_drugs), np.asarray(test_prots), np.asarray(test_Y)
#
#     smile_graph = {}
#     for smile in train_drugs:
#         g = smile2graph(smile)
#         smile_graph[smile] = g
#     prot_graph={}
#     for k in train_prot_id:
#         p=sequence2graph(distance_dir,k)
#
#     train_data = DTADataset(root='datasets/datasets/datasets', dataset=dataset + '_' + 'train', drug_smiles=train_drugs, target_sequence=train_prots, y=train_Y,
...
#                            smile_graph=smile_graph, target_graph=target_graph)
#
#     return train_data,test_data,train_data_pro,test_data_pro
