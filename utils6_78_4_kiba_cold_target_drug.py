...
class DTADataset_pro(InMemoryDataset):
    ...
    def processed_file_names(self):
        ...
        return self.dataset + '_data_pro_graph_3_kiba_cold_target_drug.pt'
...
class DTADataset(InMemoryDataset):
    def __init__(self, root='datasets/datasets/datasets', dataset='kiba',
    ...
    @property
    def processed_file_names(self):
        return self.dataset + '_data_mol_78_1_kiba_cold_target_drug.pt'
...
class DTADataset_pro_3d(InMemoryDataset):
    def __init__(self, root='datasets/datasets/datasets', dataset='kiba',
                 drug_smiles=None, target_sequence=None,target_key=None, x=None, x_mask=None, xt=None, xt_mask=None, y=None,
                 transform=None,
                 pre_transform=None, smile_graph=None, target_graph=None):
        ...
        self.smile_graph = smile_graph
        self.target_graph = target_graph
        # self.process(drug_smiles, target_sequence, target_key,x, x_mask, xt, xt_mask, y, smile_graph, target_graph)
        if os.path.isfile(self.processed_paths[0]):
            print('Pre-processed data found: {}, loading ...'.format(self.processed_paths[0]))
            self.data, self.slices= torch.load(self.processed_paths[0])
        else:
    ...
    @property
    def processed_file_names(self):
        return self.dataset +'_data_pro_3d_3_kiba_cold_target_drug.pt'
    ...
    def process(self, drug_smiles=None, target_sequence=None,target_key=None, x=None, x_mask=None, xt=None, xt_mask=None, y=None,
                smile_graph=None, target_graph=None):
        assert (len(drug_smiles) == len(target_sequence) and len(drug_smiles) == len(
            y)), 'The three lists must have the same length!'
        ...
        for i in tqdm(range(data_len)):
            ...
            # target_seq = PROTEIN2INT(tar_seq)
            target_seq_len = 1000
            pro_seq_emb = seq_cat(tar_seq, target_seq_len)
...
class DTADataset_3d(InMemoryDataset):
    def __init__(self, root='datasets/datasets/datasets', dataset='kiba',
                 drug_smiles=None, target_sequence=None,target_key=None, x=None, x_mask=None, xt=None, xt_mask=None, y=None,
                 transform=None,
                 pre_transform=None, smile_graph=None, target_graph=None):
        super(DTADataset_3d, self).__init__(root, transform, pre_transform)
        self.dataset = dataset
        self.drug_smiles = drug_smiles
        self.target_sequence = target_sequence
        self.target_key=target_key
    ...
    @property
    def processed_file_names(self):
        return self.dataset + '_data_pro_3d_3_kiba_cold_drug.pt'
    ...
    def process(self, drug_smiles=None, target_sequence=None,target_key=None, x=None, x_mask=None, xt=None, xt_mask=None, y=None,
        ...
        for i in tqdm(range(data_len)):
            ...
            labels = y[i]
            # mol_size, mol_features, mol_edge_index, mol_edge_attr= smile_graph[smiles]
            target_size,X_ca_3d,node_s_3d,node_v_3d,edge_s_3d,edge_v_3d,edge_index_3d = target_graph[tar_key]



            GCNData_pro = DATA.Data(x_3d=X_ca_3d,
                                    edge_index_3d=edge_index_3d,
                                    node_s_3d=node_s_3d,edge_s_3d=edge_s_3d,
                                    node_v_3d=node_v_3d,edge_v_3d=edge_v_3d
                                    )
